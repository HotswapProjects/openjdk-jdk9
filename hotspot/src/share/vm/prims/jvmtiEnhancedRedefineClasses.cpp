...
VM_EnhancedRedefineClasses::VM_EnhancedRedefineClasses(jint class_count,
  ...
  for (int i = 0; i < class_count; i++) {
    ...
    tty->print_cr("Hello: %s, " INTPTR_FORMAT, ik->name()->as_C_string(), ik->new_version());
  }
}
...
void VM_EnhancedRedefineClasses::lock_classes() {
  return; // FIXME: clean...
...
void VM_EnhancedRedefineClasses::unlock_classes() {
  return; // FIXME: clean...
  MutexLocker ml(RedefineClasses_lock);
  for (int i = 0; i < _class_count; i++) {
...
// FIXME: ... inline?
void swap_marks(oop first, oop second) {
...
void VM_EnhancedRedefineClasses::doit() {
  ...
  for (int i = 0; i < _scratch_classes->length(); i++) {
    redefine_single_class(_scratch_classes->at(i), thread);
  }
  ...
  // FIXME: RC_TRACE(0x00000001, ("Before updating instances"));
  {
  ...
  // FIXME: RC_TRACE(0x00000001, ("After updating instances"));

  for (int i = 0; i < _scratch_classes->length(); i++) {
    InstanceKlass* cur = InstanceKlass::cast(_scratch_classes->at(i));
    InstanceKlass* old = InstanceKlass::cast(cur->old_version());
  ...
  for (int i = 0; i < _scratch_classes->length(); i++) {
    Klass* cur_klass = _scratch_classes->at(i);
...
void VM_EnhancedRedefineClasses::doit_epilogue() {
  ...
  delete _scratch_classes;
  _scratch_classes = NULL;
  delete _affected_klasses; // FIXME: rename to _classes?
  _affected_klasses = NULL;
...
// appended.
void VM_EnhancedRedefineClasses::append_entry(const constantPoolHandle& scratch_cp,
...
int VM_EnhancedRedefineClasses::find_or_append_indirect_entry(const constantPoolHandle& scratch_cp,
...
void VM_EnhancedRedefineClasses::append_operand(const constantPoolHandle& scratch_cp, int old_bs_i,
...
int VM_EnhancedRedefineClasses::find_or_append_operand(const constantPoolHandle& scratch_cp,
...
void VM_EnhancedRedefineClasses::finalize_operands_merge(const constantPoolHandle& merge_cp, TRAPS) {
...
jvmtiError VM_EnhancedRedefineClasses::compare_and_normalize_class_versions(
...
int VM_EnhancedRedefineClasses::find_new_index(int old_index) {
...
int VM_EnhancedRedefineClasses::find_new_operand_index(int old_index) {
...
bool VM_EnhancedRedefineClasses::is_unresolved_class_mismatch(const constantPoolHandle& cp1,
...
jvmtiError VM_EnhancedRedefineClasses::load_new_class_versions(TRAPS) {
  ...
  _scratch_classes = new (ResourceObj::C_HEAP, mtInternal) GrowableArray<Klass*>(_class_count, true);
  ...
  for (int i = 0; i < _affected_klasses->length(); i++) {
    ...
    Klass* k = SystemDictionary::resolve_from_stream(the_class_sym,
                                                the_class_loader,
                                                protection_domain,
                                                &st,
                                                the_class,
    ...
    // FIXME: ...
    //k->set_revision_number(_revision_number);
    //k->set_redefinition_index(i);
    the_class->set_new_version(scratch_class());
    _scratch_classes->append(scratch_class());

    int redefinition_flags = Klass::NoRedefinition;
    if (not_changed) {
      redefinition_flags = Klass::NoRedefinition;
    } else {
      redefinition_flags = calculate_redefinition_flags(scratch_class);
      if (redefinition_flags >= Klass::RemoveSuperType) {
        return JVMTI_ERROR_UNSUPPORTED_REDEFINITION_HIERARCHY_CHANGED;
      }
    }

    if (scratch_class->super() != NULL) {
      redefinition_flags = redefinition_flags | scratch_class->super()->redefinition_flags();
    }

    for (int j = 0; j < scratch_class->local_interfaces()->length(); j++) {
      redefinition_flags = redefinition_flags | (scratch_class->local_interfaces()->at(j))->redefinition_flags();
    }

    scratch_class->set_redefinition_flags(redefinition_flags);
    ...
    if ((redefinition_flags & Klass::ModifyInstances) != 0) {
      ...
      // FIXME: instance update!!!
      calculate_instance_update_information(_scratch_classes->at(i));
    } else {
      // Fields were not changed, transfer special flags only
      assert(scratch_class->layout_helper() >> 1 == scratch_class->old_version()->layout_helper() >> 1, "must be equal");
      assert(scratch_class->fields()->length() == InstanceKlass::cast(scratch_class->old_version())->fields()->length(), "must be equal");
...
void VM_EnhancedRedefineClasses::rollback() {
  ...
  for (int i = 0; i < _scratch_classes->length(); i++) {
    SystemDictionary::remove_from_hierarchy(_scratch_classes->at(i));
  }

  for (int i = 0; i < _scratch_classes->length(); i++) {
    instanceKlassHandle new_class = _scratch_classes->at(i);
    new_class->set_redefining(false);
    new_class->old_version()->set_new_version(NULL);
    new_class->set_old_version(NULL);
  }
  _scratch_classes->clear();
}

// Map old_index to new_index as needed. scratch_cp is only needed
// for log calls.
void VM_EnhancedRedefineClasses::map_index(const constantPoolHandle& scratch_cp,
...
void VM_EnhancedRedefineClasses::map_operand_index(int old_index, int new_index) {
...
bool VM_EnhancedRedefineClasses::merge_constant_pools(const constantPoolHandle& old_cp,
...
// constant pool.
jvmtiError VM_EnhancedRedefineClasses::merge_cp_and_rewrite(
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs(instanceKlassHandle scratch_class,
       TRAPS) {
...
// Rewrite constant pool references in the methods.
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_methods(
...
void VM_EnhancedRedefineClasses::rewrite_cp_refs_in_method(methodHandle method,
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_class_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_annotations_typeArray(
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_annotation_struct(
       AnnotationArray* annotations_typeArray, int &byte_i_ref, TRAPS) {
...
u2 VM_EnhancedRedefineClasses::rewrite_cp_ref_in_annotation_data(
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_element_value(
       AnnotationArray* annotations_typeArray, int &byte_i_ref, TRAPS) {
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_fields_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_methods_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_methods_parameter_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_methods_default_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_class_type_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_fields_type_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_methods_type_annotations(
       instanceKlassHandle scratch_class, TRAPS) {
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_type_annotations_typeArray(
...
// }
//
bool VM_EnhancedRedefineClasses::rewrite_cp_refs_in_type_annotation_struct(
       AnnotationArray* type_annotations_typeArray, int &byte_i_ref,
       const char * location_mesg, TRAPS) {
...
// } target_info;
//
bool VM_EnhancedRedefineClasses::skip_type_annotation_target(
       AnnotationArray* type_annotations_typeArray, int &byte_i_ref,
       const char * location_mesg, TRAPS) {
...
// }
//
bool VM_EnhancedRedefineClasses::skip_type_annotation_type_path(
...
// }
//
void VM_EnhancedRedefineClasses::rewrite_cp_refs_in_stack_map_table(
...
// }
//
void VM_EnhancedRedefineClasses::rewrite_cp_refs_in_verification_type_info(
...
void VM_EnhancedRedefineClasses::set_new_constant_pool(
...
void VM_EnhancedRedefineClasses::transfer_old_native_function_registrations(instanceKlassHandle the_class) {
...
void VM_EnhancedRedefineClasses::swap_annotations(instanceKlassHandle the_class,
                                          instanceKlassHandle scratch_class) {
...
void VM_EnhancedRedefineClasses::redefine_single_class(Klass* scratch_class_oop, TRAPS) {
  ...
  instanceKlassHandle the_class(THREAD, scratch_class_oop->old_version());
  assert(the_class.not_null(), "must have old version");
...
*/
  ...
  */
  ...
  if (mnt != NULL) {
    ...
    scratch_class->set_member_names(mnt);
...
jvmtiError VM_EnhancedRedefineClasses::find_sorted_affected_classes(TRAPS) {
  for (int i = 0; i < _class_count; i++) {
    instanceKlassHandle klass_handle(THREAD, get_ik(_class_defs[i].klass));
...
jvmtiError VM_EnhancedRedefineClasses::do_topological_class_sorting(TRAPS) {
  ...
  for (int i = 0; i < _class_count; i++) {
    instanceKlassHandle klass(THREAD, get_ik(_class_defs[i].klass));
    ...
    if (super_klass != NULL && _affected_klasses->contains(const_cast<Klass*>(super_klass))) {
      links.append(KlassPair(super_klass, klass()));
    }

    Array<Klass*>* local_interfaces = parser.local_interfaces();
    for (int j = 0; j < local_interfaces->length(); j++) {
      const Klass* iface = local_interfaces->at(j);
      if (iface != NULL && _affected_klasses->contains(const_cast<Klass*>(iface))) {
